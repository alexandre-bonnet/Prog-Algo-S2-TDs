...
std::unordered_map<IDType, std::pair<float, IDType>> Dijkstra(WeightedGraph const& graph, IDType const start, IDType const end) {
    ...
    distances.insert({start, {0.0f,start}});
    ...
    while(!to_visit.empty()){
        ...
        IDType currentID = current.second;
        ...
        for (auto const& neighbour : graph.get_neighbors(currentID)) {
            IDType neighborID = neighbour.to;
            float addedCost = currentCost + neighbour.weight;

            if (!distances.contains(neighborID) || addedCost < distances[neighborID].first) {
                distances[neighborID] = {addedCost, currentID};
                to_visit.push({addedCost, neighborID});
            }
        }
    }
